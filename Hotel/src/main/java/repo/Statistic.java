...
public class Statistic
{
 ...
 { 
  ...
  switch(i)
  {
   case 0 : createMonthRaport(mFrom,mTo,yFrom);break;
   case 1 : createYearRaport(yFrom,yTo);break;
   case 2 : break;
  } 
 }
 ...
 { 
  ...
  switch(i)
   {
	case 0 : createClassRaport(month,year);break;
	case 1 : createRoomRaport(month,year,classRoom);break;
	case 2 : createServesRaport(month,year);break;
	case 3 : createServeRaport(month,year,serve);break;
   } 	
 }
 ...
 /*
 private void createArray(int n,int m)
 {
  int i=0,j=0;	 
...
*/
 ...
 private void createClassRaport(int month,int year)
 {
  ...
  try
  {
   ...
   query+="WHERE MONTH(r.data_w)="+month+" and YEAR(r.data_w)="+year+" GROUP BY k.id_klasy ORDER BY k.id_klasy";
   ...
   if(resultQuery!=null)
    {
	 ...
	 if(n!=0) array=new double[n][2];
	 System.out.println("count row: "+n);///
	 resultText="Raportu z wykorzystania klas pokoi za miesi�c "+monthTab[month]+" w roku "+year+"\n";
     while(resultQuery.next())
      {
       resultText+="   Klasa "+resultQuery.getString(1)+"("+i+").\n";
       m=resultQuery.getInt(2);
       array[i][0]=resultQuery.getFloat(3);
       resultText+="\tzyski: "+String.format("%.2f",array[i][0])+"z�\n";
    ...
    else resultText+="Wyst�pi� b�ad bazy danych\n";
   if(n==0) resultText+="W danym miesi�cu nie wprowadzano �adnych danych za zakresu.\n"; 
  }
 ...
 private void createRoomRaport(int month,int year,String classRoom)
 {
  ...
  try
  {
   ...
   query+="FROM rezerwacje r ";
   query+="JOIN pokoje p ON r.id_pokoju=p.id_pokoju  JOIN klasy k ON p.id_klasy=k.id_klasy "; 
   query+="WHERE MONTH(r.data_w)="+month+" and YEAR(r.data_w)="+year+" and k.opis='"+classRoom+"' ";
   ...
   if(resultQuery!=null)
    {
	 ...
	 if(n!=0) array=new double[n][2];
	 System.out.println("count row: "+n);///
	 resultText="Raportu z wykorzystania pokoi z klasy "+classRoom+" za miesi�c "+monthTab[month]+" w roku "+year+"\n";
     while(resultQuery.next())
      {
       resultText+="   Pok�j nr: "+resultQuery.getString(1)+"("+i+").\n";
       m=resultQuery.getInt(2);
       array[i][0]=resultQuery.getFloat(3);
       resultText+="\tzyski: "+String.format("%.2f",array[i][0])+"z�\n";
       array[i][1]=array[i][0]/m;
...
private void createServesRaport(int month,int year)
{
 ...
 try
 {
  ...
  query+="WHERE MONTH(rz.data_w)="+month+" and YEAR(rz.data_w)="+year+" GROUP BY u.typ";
  ...
  if(resultQuery!=null)
   {
	...
	if(n!=0) array=new double[n][2];
	System.out.println("count row: "+n);///
	resultText="Raportu z wykorzystania us�ug  za miesi�c "+monthTab[month]+" w roku "+year+"\n";
    while(resultQuery.next())
     {
      ...
      query+="rrz ON rrk.id_rez=rrz.id_rez WHERE uu.typ='"+resultQuery.getString(1)+"' ";
      query+="  GROUP BY rrk.id_rez ) tab";
      System.out.println("qurery("+resultQuery.getString(1)+"): "+query);
      resA=db.query(query);
      resA.next();
      m=resA.getInt(1);
...
private void createServeRaport(int month,int year,String serve)
{
 ...
 try
 {
  ...
  query+="FROM rekreacja rk JOIN uslugi u ON rk.id_uslugi=u.id_uslugi ";
  query+="JOIN rezerwacje rz ON rk.id_rez=rz.id_rez ";
  query+="WHERE MONTH(rz.data_w)="+month+" and YEAR(rz.data_w)="+year+" AND u.typ='"+serve+"' ";
  ...
  if(resultQuery!=null)
   {
    ...
    while(resultQuery.next())
     {
      resultText+="   Nazwa uslugi "+resultQuery.getString(1)+"("+i+").\n";      
      array[i][0]=resultQuery.getFloat(3);
      resultText+="\tsumaryczny czas: "+resultQuery.getInt(2)+" godzin\n";
      resultText+="\tzysk: "+String.format("%.2f",array[i][0])+"z�\n";
      query="SELECT count(*) ilosc_zameldowan FROM (SELECT count(rrk.id_rez) ilosc_rez ";
      query+="FROM rekreacja rrk JOIN uslugi uu ON rrk.id_uslugi=uu.id_uslugi JOIN rezerwacje ";
      query+="rrz ON rrk.id_rez=rrz.id_rez WHERE uu.nazwa='"+resultQuery.getString(1)+"' ";
      query+="  GROUP BY rrk.id_rez ) tab";
      System.out.println("qurery("+resultQuery.getString(1)+"): "+query);
      resA=db.query(query);
      resA.next();
...
private void createMonthRaport(int monthFrom,int monthTo,int year)
{
 ...
 try
 {
  ...
  query+="JOIN pokoje p ON r.id_pokoju=p.id_pokoju JOIN klasy  k ON p.id_klasy=k.id_klasy ";
  query+="WHERE MONTH(r.data_w)>="+monthFrom+" AND MONTH(r.data_w)<="+monthTo+" ";
  query+=" AND YEAR(r.data_w)="+year+" GROUP BY MONTH(r.data_w)";
  System.out.println("qurery: "+query);
  resultQuery=db.query(query);
  if(resultQuery!=null)
   {
	...
	n=resultQuery.getRow();  
	resultQuery.beforeFirst();
	if(n!=0) array=new double[n][4];
	if(monthFrom==monthTo) resultText="Raportu zysk�w w miesi�cu "+monthTab[monthFrom]+"\n";
	 else resultText="Raportu zysk�w w miesi�cach od "+monthTab[monthFrom]+" do "+monthTab[monthTo]+"\n";
    while(resultQuery.next())
     { 	
      resultText+="   Miesiac "+monthTab[resultQuery.getInt(1)]+"("+i+").\n";
...
private void createYearRaport(int yearFrom,int yearTo)
{
 ...
 try
 {
  ...
  query+="JOIN pokoje p ON r.id_pokoju=p.id_pokoju JOIN klasy  k ON p.id_klasy=k.id_klasy ";
  query+="WHERE YEAR(r.data_w)>="+yearFrom+" AND YEAR(r.data_w)<="+yearTo+" ";
  ...
  if(resultQuery!=null)
   {
	...
	resultQuery.beforeFirst();
	if(n!=0) array=new double[n][4];
	if(yearFrom==yearTo) resultText="Raportu zysk�w w roku "+yearFrom+"\n";
	 else resultText="Raportu zysk�w w latach od "+yearFrom+" do "+yearTo+"\n";
    while(resultQuery.next())
     {
