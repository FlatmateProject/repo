...
public class StaticticDaoImpl extends AbstractDao implements StaticticDao {
	...
	@Override
	public List<RoomData> findRoomsByType(int month, int year, String classRoom) {
	...
	@Override
	public List<ServiceTypeData> findServiceTypes(int month, int year) {
		...
		query += "WHERE MONTH(rz.data_w)=" + month + " and YEAR(rz.data_w)="
				+ year + " GROUP BY u.typ";
		return exacuteQuery(query, ServiceTypeData.class);
	}

	@Override
	public List<ServiceData> findServiceByType(int month, int year, String serve) {
		...
		return exacuteQuery(query, ServiceData.class);
	}
	...
	@Override
	public int countUseNumberForServiceType(String serveType) throws SQLException {
	...
	@Override
	public int countUseNumberForServiceName(String serveName) throws SQLException {
		String query = "SELECT count(*) ilosc_zameldowan FROM (SELECT count(rrk.id_rez) ilosc_rez ";
		query += "FROM rekreacja rrk JOIN uslugi uu ON rrk.id_uslugi=uu.id_uslugi JOIN rezerwacje ";
